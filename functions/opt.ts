...
export default async function optSwap(
  ...
  */
  ...
  console.log(`Swapped ${Number(balance) / 10**8} BTCb from ARB to OPT`);
  ...
  */
  let i = 0;
  while (i < 10) {
    const BTCbBalanceOnDest = await BTCbContract_OPT.balanceOf(wallet.address);
    if(BTCbBalanceOnDest) {
      results[id].btcb_dest = `${Number(balance) / 10**8} OPT BTC.b`;
      console.log(`BTCb BALANCE ON OPT ${BTCbBalanceOnDest}`)
      console.log(`=============`);
      break;
    } 
