...
export default async function optSwap(
  ...
  */
  ...
  console.log(`Swapped ${Number(balance) / 10**8} BTCb from AVAX to OPT`);
  ...
  */
  let i = 0;
  while (i < 10) {
    const BTCbBalanceOnDest = await BTCbContract_AVAX.balanceOf(wallet.address);
    if(BTCbBalanceOnDest) {
      results[id].btcb_dest = `${Number(balance) / 10**8} AVAX BTC.b`;
      console.log(`BTCb BALANCE ON AVAX ${BTCbBalanceOnDest}`)
      console.log(`=============`);
      break;
    }
